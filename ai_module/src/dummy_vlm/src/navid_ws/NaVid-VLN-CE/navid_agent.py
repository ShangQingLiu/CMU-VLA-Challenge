...
from remote_agent_proxy import RemoteNaVidAgentProxy
...
class VLM:
    def __init__(self):
        ...
        self.client = RemoteNaVidAgentProxy(host="0.0.0.0", port=8000, timeout=60)
        random.seed(42)

        self._instr_evt = threading.Event()
    ...
    def evaluate_agent_CMU(self, model_path, result_path) -> None:   
        ...
        if not self._instr_evt.wait(timeout=10.0):
            rospy.logwarn("No /challenge_question within 10s; skip this evaluate.")
            return None
        ...
        result = self.client.act(self.obs, None, "CMU")
